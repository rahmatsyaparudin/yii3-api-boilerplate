...
final readonly class RequestParams
{
    ...
    public function __construct(
        ...
        if (!\is_numeric($pageParam)) {
            throw new BadRequestException(translate: Message::create(key: 'pagination.invalid_parameter', params: ['parameter' => 'page']));
        }
        $page = \max(self::DEFAULT_PAGE, (int) $pageParam);

        if (!\is_numeric($pageSizeParam)) {
            throw new BadRequestException(translate: Message::create(key: 'pagination.invalid_parameter', params: ['parameter' => 'page_size']));
        }
    ...
    public function ensureExists(string $resource): void
    {
        if (empty($this->rawParams->all())) {
            throw new BadRequestException(translate: Message::create(key: 'validation.invalid_payload', params: ['resource' => $resource]));
        }
    }
    ...
    public static function fromRequest(ServerRequestInterface $request, string $attribute = 'payload'): self
    {
        ...
        if (!$params instanceof self) {
            throw new BadRequestException(translate: Message::create(key: 'http.missing_request_params', params: ['parameter' => 'page']));
        }
