...
trait ManagesPersistence
{
    ...
    public function verifyLockVersion(object $entity, ?int $version): void
    {
        ...
        if (!$entity->getLockVersion()->equals(LockVersion::fromInt($version))) {
            throw new OptimisticLockException(translate: Message::create(key: 'optimistic.lock.failed', params: ['resource' => \defined('static::RESOURCE') ? static::RESOURCE : 'resource', 'version' => $version]));
        }
    }
    ...
    {
        $resourceName = \defined($entity::class . '::RESOURCE') ? $entity::RESOURCE : 'resource';

        // Jika kita ingin mengecek lock dan lock aktif
        if ($checkLock && $this->isOptimisticLockEnabled($entity)) {
            throw new OptimisticLockException(translate: Message::create(key: 'optimistic.lock.failed', params: ['resource' => $resourceName]));
        }

        throw new NotFoundException(translate: Message::create(key: 'resource.not_found', params: ['resource' => $resourceName, 'id' => $entity->getId()]));
    }
