...
abstract class AbstractSeederData
{
    ...
    public function __construct(
        ...
        if (!\defined($yamlFile) || empty(\constant($yamlFile))) {
            throw new BadRequestException(translate: Message::create(key: 'seeder.missing_yaml_file', params: ['class' => static::class]));
        }

        $entityClass = static::class . '::ENTITY_CLASS';
        if (!\defined($entityClass) || empty(\constant($entityClass))) {
            throw new BadRequestException(translate: Message::create(key: 'seeder.missing_entity_class', params: ['class' => static::class]));
        }
    }
